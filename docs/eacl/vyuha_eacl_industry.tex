...
\begin{axis}[ybar, width=\columnwidth, height=4.2cm, font=\scriptsize, bar width=5pt,
  ...
  enlarge x limits=0.18, legend style={at={(0.5,1.13)},anchor=south,legend columns=2,draw=none}]
...
\end{axis}
\end{tikzpicture}
\caption{Adaptive-attack ablation (C4), 150 in-the-wild seeds. L0 normalisation alone cuts
\emph{static} ASR to 0.02 but the adaptive attacker still reaches 1.00; adversarial augmentation is
the load-bearing fix, closing adaptive ASR to 0.03 at 1\% FPR.}
\label{fig:adaptive}
\end{figure}
...
\addplot[fill=blue!55,draw=blue!55] coordinates {(1,1.00)(2,0.927)(3,0.900)(4,0.855)(5,0.680)(6,0.609)};
\addplot[fill=orange!75,draw=orange!75] coordinates {(7,0.193)(8,0.073)};
...
(BeaverTails) it sits near the benchmark's 4B leader (0.84, dashed); the two toxicity-prefix axes
(RealToxicityPrompts) are short fragments a request-focused guard flags less.}
\label{fig:nist}
\end{figure}
...
why L2 composes guards. Composing a second, non-overlapping guard (Granite Guardian) in an OR union
confirms the benchmark's ensembling recommendation --- recall rises across the board (overall
$0.551\!\to\!0.715$; the weak toxicity axes $0.133\!\to\!0.373$) --- but the gain is not free: the
naive union also raises benign FPR from 0.064 to 0.48, because the second guard over-flags this benign
set at its default operating point. A deployable union must therefore run at per-member thresholds
(fixed-FPR calibration), not a raw OR.
