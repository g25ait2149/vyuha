...
0.6B content guard reaches 0.799 recall on complete-harmful-request axes --- level with a 7$\times$
...
Vyuha is a pipeline of six layers, each owning a different failure mode and escalating only when
needed (a \emph{selective cascade} that keeps per-request cost low).
...
compositions of known evasions, and ablate each design choice, rather than reporting a static
per-mutator number that understates the attack surface. \textbf{(4) Standardised benchmark.} We score
...
Adaptive (L1) & ASR, L0 only & 1.00 [0.97,1.00] \\
 & ASR, L0+aug & \textbf{0.03} [0.01,0.08] \\
Semantic (L2) & PAIR recall & \textbf{0.90} (L1: 0.07) \\
NIST-RMF (L2) & recall, req.\ axes & \textbf{0.799} ($\approx$4B: 0.84) \\
 & benign FPR & 0.064 \\
...
\paragraph{Adaptive robustness (C4).} Against an adaptive attacker (exhaustive search over 101
...
to 0.03 [0.01,0.08] at 1\% benign FPR. The two are complementary --- augmentation alone leaves 0.25
[0.19,0.33], a non-overlapping interval --- so both design choices earn their place.
...
\paragraph{NIST-AI-RMF guard benchmark (L2).} Scoring the 0.6B content guard on the 8 NIST-AI-RMF
categories \citep{guard-bench} (a reconstruction from BeaverTails and RealToxicityPrompts;
807 unsafe / 800 benign; not the official split): recall is 0.799 on the six
complete-harmful-request axes --- level with the benchmark's best model (Qwen Guard 4B, 0.840)
despite being $\sim$7$\times$ smaller --- and 0.133 on the two toxicity-prefix axes (short fragments a
safety guard reasonably flags less), for an aggregate 0.551 [0.52,0.58] at 6.4\% benign FPR. A small
guard is competitive with a much larger one on complete harmful requests; the per-category spread is
why L2 composes guards.
...
\texttt{/guard\_turn}), and a CLI, so a team can drop it in front of or behind an LLM. The selective
cascade bounds per-request cost by keeping the guard off the hot path; thresholds are tuned to the
deployment's benign base rate. For governance, layers map to the OWASP LLM Top~10 (LLM01 prompt
injection $\rightarrow$ L0--L3; LLM02/LLM07 disclosure/leakage $\rightarrow$ L4; LLM06 excessive
agency $\rightarrow$ L3) and to the NIST AI RMF Measure/Manage functions, with out-of-scope risks
named rather than papered over. Continuous red-teaming and a drift monitor support operational
maintenance as the attack mix shifts.

\section{Related Work and Conclusion}
Vyuha's obfuscation robustness is shared with strong neural guardrails; its defensible edge is cost,
reproducibility, composition robustness, and honest measurement. Content guards are stronger on
semantics (we compose one); CaMeL \citep{camel} offers capability guarantees our black-box L3 only
approximates; representation-level defenses report lower residual ASR but require model access. We
conclude that a credible, modern LLM guardrail can be assembled from independent, individually cheap
layers entirely on free compute, and --- crucially --- measured honestly, with confidence intervals,
adaptive attacks, and a standardised benchmark, so that its real strengths and limits are visible.
...
Crescendo result is on modeled score trajectories. Some efficiency and attack-cost figures are
...
\bibitem[OWASP(2025)]{owasp-llm} OWASP. 2025. \emph{OWASP Top 10 for Large Language Model Applications}.
\bibitem[Anonymous(2025)]{attacker-second} 2025. The Attacker Moves Second: adaptive attacks bypass recent LLM defenses. OpenReview.
\bibitem[Russinovich et~al.(2025)]{crescendo} Russinovich et~al. 2025. Great, Now Write an Article About That: The Crescendo Multi-Turn Jailbreak. USENIX Security.
\bibitem[Harsh et~al.(2026)]{guard-bench} Harsh, Sarmah, and Pasquali. 2026. Benchmarking Open-Source Safety Guard Models: A Comprehensive Evaluation. arXiv:2605.28830.
\bibitem[Debenedetti et~al.(2025)]{camel} Debenedetti et~al. 2025. Defeating Prompt Injections by Design (CaMeL).
\end{thebibliography}
